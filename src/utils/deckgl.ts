import { Deck, MapView, View } from "@deck.gl/core";
import type { DeckProps, ViewStateMap } from "@deck.gl/core";
...
export class DeckLayer extends L.Layer {
  ...
  pickObject(
    opts: Parameters<Deck["pickObject"]>[0],
  ): ReturnType<Deck["pickObject"]> {
    if (!this._deck) {
      return null;
    }
  ...
  pickObjects(
    opts: Parameters<Deck["pickObjects"]>[0],
  ): ReturnType<Deck["pickObjects"]> {
    if (!this._deck) {
      return [];
    }
  ...
  #updateTransform(center: L.LatLng, zoom: number): void {
    ...
    const currentCenterPoint = this.#getMap().project(
